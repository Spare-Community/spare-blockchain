...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in SPARE_ROOT.
    spare_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within SPARE_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    if args.plotter == "chiapos":
        plot_chia(args, spare_root_path)
    if args.plotter == "madmax":
        plot_madmax(args, spare_root_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, spare_root_path, root_path)
